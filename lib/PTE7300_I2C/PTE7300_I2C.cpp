...
PTE7300_I2C::PTE7300_I2C(TwoWire* i2c)
{
  _i2c = i2c;
...
{
	_i2c->beginTransmission(_nodeAddress);
    if (_i2c->endTransmission() == 0) {return true;}
	else { return false;}
}
...
unsigned int PTE7300_I2C::readRegisterNoCRC(uint8_t address, unsigned int number, uint16_t *buffer)
{
  ...
  _i2c->beginTransmission(_nodeAddress);
  _i2c->write(address); //Send register address
  _i2c->endTransmission();
  _i2c->requestFrom(_nodeAddress, number * 2); //Request register, note that register is 2 bytes wide

  bytesRead = _i2c->available();
  if ( bytesRead >= number * 2 )
  {
    for (int i = 0; i < number; i++)
    {
      byte lowByte = _i2c->read(); // read low byte
	  byte highByte = _i2c->read(); // read high byte
...
{	
  ...
  _i2c->beginTransmission(_nodeAddress | 1); //indicate CRC-transmission by setting first address bit to 1
  _i2c->write(address); //Send register address
  _i2c->write((((number*2)-1) << 4) | (crc4 & 0x0F));
  _i2c->endTransmission();
  _i2c->requestFrom(_nodeAddress | 1,(number*2)+1); //Request registers, note that registers 2 bytes wide
  ...
  int crc8_received = _i2c->read(); // read CRC byte, after reading the databuffer words
...
void PTE7300_I2C::writeRegisterNoCRC(uint8_t address, unsigned int number, uint16_t* data)
{
	...
	for (int i = 0; i < number; i++)
	{
		_i2c->write(data[i] & 0x00FF); //write low byte
		_i2c->write((data[i] & 0xFF00) >> 8); // write high byte
	}
	_i2c->endTransmission();
}
